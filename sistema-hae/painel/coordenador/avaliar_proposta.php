...
$sql = "SELECT i.id, i.titulo, i.resumo, u.nome AS professor, u.curso, i.status, i.observacoes 
        FROM inscricoes i 
...
<body>
  ...
  <div class="container">
    <?php while ($p = $result->fetch_assoc()): ?>
      <form method="POST" action="salvar_avaliacao.php" class="projeto-card">
        <h2><?= htmlspecialchars($p['titulo']) ?></h2>
        <p class="info"><strong>Professor:</strong> <?= htmlspecialchars($p['professor']) ?></p>
        <p class="info"><strong>Curso:</strong> <?= htmlspecialchars($p['curso']) ?></p>
        <p class="info"><strong>Resumo:</strong> <?= htmlspecialchars($p['resumo']) ?></p>
        ...
        <label for="comentario_<?= $p['id'] ?>">Comentário da Coordenação:</label>
        <textarea name="comentario" id="comentario_<?= $p['id'] ?>" placeholder="Escreva sua justificativa ou observações..."><?= htmlspecialchars($p['observacoes'] ?? '') ?></textarea>
